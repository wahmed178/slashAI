...
function ResourceSteps({ steps }: { steps: string[] }) {
  return (
  ...
  return (
    <section className="mt-6">
      ...
      <p className="mt-2 text-xs text-muted-foreground">
        Metadata is curated manually; anything we could not verify is left out rather than guessed.
      </p>
    </section>
  );
}
...
function ResourcePage() {
  ...
  return (
    ...
    >
      <ResourceBreadcrumbs resource={resource} sectionLabel={def?.label} />
      ...
      {resource.steps && <ResourceSteps steps={resource.steps} />}
